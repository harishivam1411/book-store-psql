...
class CategoryCreate(BaseModel):
    ...
    description: str = Field(..., examples=['Fictional stories set in the past that often incorporate real historical events'])
...
class CategoryUpdate(BaseModel):
    ...
    description: Optional[str] = Field(
        None,
        examples=[
            "Updated description: Literary works created from the imagination, including novels, short stories, and plays."
        ],
    )
...
class CategoryResponse(CreateUpdateSchema):
    ...
    description: str = Field(..., examples=['Fiction description'])
    book_count: int = Field(0, examples=[8])
    top_books: list[TopBooksSchema] = Field([], examples=[[
